...
public class LoginActivity extends Activity implements LoaderCallbacks<Cursor> {
    ...
    public class UserLoginTask extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
            ...
            // run a query
          //  Cursor cursor = db.rawQuery("select count(PASSWORD) from "+InClassDatabaseHelper.TABLE_NAME+"where EMAIL='"
           //                             +mEmail+"' and PASSWORD='" +mPassword+"'", null);
         //   Cursor cursor = db.rawQuery("select count(PASSWORD) from "+InClassDatabaseHelper.TABLE_NAME+"where EMAIL='" +mEmail+"'+ and PASSWORD='" +mPassword+"'", null);
            if (!TextUtils.isEmpty(mEmail)) {
                Cursor cursor = db.rawQuery("select PASSWORD from " + InClassDatabaseHelper.TABLE_NAME + " where EMAIL='" + mEmail + "'", null);
                cursor.moveToFirst();
                if (cursor.getCount() > 0) {
                    String pass = cursor.getString(0);
                    if (!TextUtils.isEmpty(pass) && pass.equals(mPassword)) {
                        bStatus = true;
                    } else {
                        //incorrect password
        ...
        protected void onPostExecute(final Boolean success) {
            ...
            if (success) {
        //        finish();
                Intent yourIntent = new Intent(LoginActivity.this, Activity2.class);
                Bundle b = new Bundle();
